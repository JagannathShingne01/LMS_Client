function CarouselSlide({image, title, description, slideNumber, totalSlides}){
    return(
        ...
        <div className="flex flex-col items-center justify-center  lg:px-[15%]">
            ...
            <p className="text-lg mx-2 md:text-xl  text-center text-black">{description}</p>
            <div className="absolute flex justify-between transform -translate-y-1/2 left-5 right-5 md:left-5 md:right-5 top-[40%] md:top-1/2 ">
                <a href={`#slide${(slideNumber == 1 ? totalSlides : (slideNumber - 1))}`} className="btn btn-circle text-white text-2xl bg-primary">❮</a> 
                <a href={`#slide${(slideNumber) % totalSlides + 1}`} className="btn btn-circle text-white text-2xl bg-primary">❯</a>
            </div>
        </div>
    </div> 
    )
